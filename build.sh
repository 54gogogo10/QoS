...
# 临时降级 go.mod（Win7 需要 Go 1.20 与旧依赖）；结束后恢复
...
CGO_ENABLED=1 GOOS=windows GOARCH=amd64 go mod tidy > /dev/null 2>&1
CGO_ENABLED=1 GOOS=windows GOARCH=amd64 go build -trimpath -ldflags "-s -w" -o qostool-win7.exe ./cmd/qostool
...
cp qostool-app.exe qostool.exe qostool-linux-amd64 qostool-linux-arm64 qostool-win7.exe tools/WebView2Loader.dll configs/example.yaml README.md "dist/$VERSION/"
VERSION="$VERSION" python - <<'PYEOF'
import zipfile, os
